...
\begin{align}
    s\equiv \frac{v}{M_V^2}\qquad v=0.246\text{ TeV}\\
...
The remaining scalar and tensor LEFT interactions considered in this
work originate at dimension seven in the SMEFT and violate lepton
number by two units. Their final states do not interfere with the Standard-Model amplitude. Writing
$(z_7=v^3C_7)$, their leading contribution to a Michel observable
therefore scales parametrically as:
\begin{equation}
\Delta\mathcal{O}\sim |z_7|^2
\sim\left(\frac{v}{\Lambda}\right)^6 .
\end{equation}
This explains why the corresponding scale sensitivity is only of
order $(0.4\text{--}1,\mathrm{TeV})$, despite the high precision of
the Michel-parameter measurements. 

The scales collected in Tab.~\ref{tab:smeft-rge-primary} should be
understood as coefficient scales,
\begin{equation}
\Lambda_{\mathrm{est}}
=\frac{v}{|v^3C_7|^{1/3}},
\end{equation}
obtained by assuming a unit coefficient in
(C_7=1/\Lambda^3). They are not model-independent lower bounds on
the mass of an individual mediator. In particular,
(L_{\nu e,1112}^{S,LL}) constrains only the combination
(3z_S-2z_M). The symmetric and mixed-symmetry rows of
Tab.~\ref{tab:smeft-rge-primary} correspond to the mutually exclusive
hypotheses (z_M=0) and (z_S=0), respectively; they cannot be
interpreted as simultaneous bounds. Since the likelihood is even in
this lepton-number-violating coefficient, the mildly preferred
non-zero magnitude also has two sign-degenerate branches. As discussed
in Sec.~\ref{sec:left-constraints}, this preference reflects tensions
among the present Michel-parameter inputs and does not constitute
evidence for lepton-number violation.

The evolution between the electroweak and estimated UV scales changes
the seeded dimension-seven coefficients by at most approximately
(6\%), while their mixing into other dimension-seven directions
remains below the per-mille level. Thus the short RGE evolution does
not qualitatively rotate the constrained SMEFT directions. A more
consequential effect is their mixing into the dimension-five Weinberg
operator. If one imposed (C_5(\Lambda)=0) at the high scale and
allowed the dimension-seven coefficients to saturate their
muon-decay bounds, downward evolution would naively generate
Majorana-mass matrix entries of order
(10^2\text{--}10^4,\mathrm{eV}), far above the observed sub-eV
neutrino-mass scale. The large naive scales demonstrate that neutrino-mass
observables can be considerably more sensitive to these directions
than muon decay unless the mixing is symmetry-suppressed or cancelled
by an independent Weinberg coefficient
\parencite{liao2025rge,katrin2025direct}.

The UV interpretation introduces further correlations. The single-field
completions (F_1) and (F_5) are the singlet and triplet Majorana
fermions familiar from type-I and type-III seesaw constructions. Their
renormalizable interactions generally generate the Weinberg operator
already at tree level, while the matching to
(\mathcal O_{LeHD}) additionally contains a charged-lepton Yukawa
coupling. Consequently the unit-normalized
(\Lambda_{\mathrm{est}}) can substantially overestimate the actual
mass reach of muon decay in these completions. More generally, for
\begin{equation}
C_7=\frac{\kappa\prod_i\lambda_i}{M^3},
\end{equation}
the inferred constraint is
\begin{equation}
M>\Lambda_{\mathrm{est}}
\left|\kappa\prod_i\lambda_i\right|^{1/3}.
\end{equation}
For a two-field completion, muon decay instead constrains a
coupling-weighted product of masses, such as (M_XM_Y^2), or a
combination involving a dimensionful trilinear coupling. A bound on
either mass separately therefore requires additional assumptions
about the UV spectrum. The principal dimension-seven result of this
analysis is thus not a universal mediator exclusion, but the
identification of the lepton-number-violating SMEFT directions to
which muon decay is sensitive and their non-trivial connection to
neutrino mass.
