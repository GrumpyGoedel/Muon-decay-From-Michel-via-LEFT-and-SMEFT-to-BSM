...
\begin{itemize}
    \item \textbf{A complete, process-independent operator basis:}
    The SPVAT formalism parametrizes the most general local,
    derivative-free four-fermion interactions relevant to muon decay. LEFT instead classifies all independent interactions among the light fields through a chosen mass dimension,
    with their flavor structure and baryon- and lepton-number properties made explicit.
    \item \textbf{Systematic power counting and truncation:}
    LEFT organizes the interactions according to their canonical mass
    dimension and thus according to inverse powers of the heavy scales.
    This provides a controlled prescription for retaining all independent
    operators through a given order and neglecting higher-dimensional
    contributions consistently.
    \item \textbf{Matching and renormalization-group (RG) evolution:}
    The LEFT Wilson coefficients form a renormalized operator basis with
    known QED and QCD anomalous dimensions. They can therefore be evolved
    between the muon and electroweak scales while accounting for operator
    mixing, and can subsequently be matched onto SMEFT coefficients.
\end{itemize}
...
\begin{align}
    \op_{\nu e}^{V,LL}&=(\bar\nu_{L,p}\gamma^\mu\nu_{L,r})(\bar e_{L,s}\gamma_\mu e_{L,t})\label{eq:OVLL}\\
    \op_{\nu e}^{V,LR}&=(\bar\nu_{L,p}\gamma^\mu\nu_{L,r})(\bar e_{R,s}\gamma_\mu e_{R,t})\\
    \op_{\nu e}^{S,LL}&=(\nu_{L,p}^TC\nu_{L,r})(\bar e_{R,s} e_{L,t})\\
    \op_{\nu e}^{S,LR}&=(\nu_{L,p}^TC\nu_{L,r})(\bar e_{L,s} e_{R,t})\\
    \op_{\nu e}^{T,LL}&=(\nu_{L,p}^TC\sigma^{\mu\nu}\nu_{L,r})(\bar e_{R,s}\sigma_{\mu\nu} e_{L,t})\label{eq:OTLL}
\end{align}
$C$ is charge conjugation matrix and $p,r,s,t$ are flavor indices. For this work we will only consider the dimension-six operators. We note that only $\op_{\nu e}^{V,LL}$ and $\op_{\nu e}^{V,LR}$ preserve lepton number and thus correspond to SM-like neutrino output. In the following section, we will perturb the SM Michel parameters with the LEFT operators from Eq.~\ref{eq:OVLL} to Eq.~\ref{eq:OTLL}.
\section{Michel parameters in the LEFT}
We will now present our calculations for the perturbation of the Michel parameters by LEFT operators of dimension six. In Sec.~\ref{sec:SMlikeNeutrinoOutput} we only consider the SM-like neutrino output $\nu_\mu+\bar\nu_e$ and in Sec.~\ref{sec:FullFourFermionResults} we allow for all combinations of two outgoing neutrinos.
\subsection{SM-like neutrino output}\label{sec:SMlikeNeutrinoOutput}
We begin by deriving the Michel parameters for $\mu^-\to e^-\bar\nu_e\nu_\mu$, which corresponds to the SM process. We follow the discussion from Sec.~\ref{sec:michel} and further elaborated in appendix~\ref{ch:kinematics}. We consider strictly the SM-like process as a simple first look at our results.
...
%
\begin{equation}
\Lambda\ge 21v\sim {\rm\ 5.1\ TeV}\label{eq:naiveresult}
\end{equation}
...
\begin{equation}
    \eta\approx\frac{v^2}{2}\text{Re}[L^{V,LR}_{\nu e,2112}] \quad\quad \frac{\beta'}{A} \approx  -\frac{v^2}{4} \text{Im}[L^{V,LR}_{\nu e,2112}]
